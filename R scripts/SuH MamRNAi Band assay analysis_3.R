...
mainDir <- "/Users/jonathantownson/Documents/PhD/Images/Analysed image data/Band assay/Truncations/MamRNAi/Su(H)"
setwd(mainDir)
# Load the background values
DF <- read.csv("/Users/jonathantownson/Documents/PhD/Images/Analysed image data/Band assay/Truncations/MamRNAi/Su(H)/Background SuH with MamRNAi levels.csv", head = TRUE, sep = ",")
BackgroundValues <- data.frame(Nucleus=DF[,1], SuHBG=rowMeans(DF[,-1]))
rm(DF)
# Create the path where the data is stored
DataPath <- "/Users/jonathantownson/Documents/PhD/Images/Analysed image data/Band assay/Truncations/MamRNAi/Su(H)/Cleaned Band assay files"
#DataPath <- "/Users/jonathantownson/Documents/PhD/Images/Analysed image data/Band assay/Truncations/MamRNAi/Su(H)/Individual Genotype comparisons csv files/NDECDOPA"
...
df <- filter(RelIntSuH,Genotype %in% c("wtNDECD","TADPEST"))
RelInt_crossbar(df,Labs) +
  geom_signif(comparisons = list(c("wtNDECD","TADPEST")), 
              map_signif_level=TRUE, test = "t.test", color = "black")

Labs <- c(title = "Fold change of band over background", x = "Genotype", y = "SuH fold change")
df <- filter(RelIntSuHBg,Genotype %in% c("wtNDECD","TADPEST"))
RelInt_crossbar(df,Labs) +
  geom_signif(comparisons = list(c("wtNDECD","TADPEST")), 
              map_signif_level=TRUE, test = "t.test", color = "black")
