...
setwd("/Users/jonathantownson/Documents/PhD/qPCR data/NDECD truncations")
# Set the path to the folder where the data csv files are stored:
DataPath <- "/Users/jonathantownson/Documents/PhD/qPCR data/NDECD truncations/Selected analysed data"
# Instruct the script what the control primers being used are
ControlPrimers <- c("RP49", "Pol II")
# Instruct the script of any Genotype ordering required e.g. control then experimental condition
XAxisOrder <- c("lacZ", "Wild Type")

#### Read in the data ####
RawData <- read.csv("/Users/jonathantownson/Documents/PhD/qPCR data/NDECD truncations/Selected analysed data.csv")
...
RawData <- RawData %>% 
  mutate(Genotype = ifelse(as.character(Genotype) == "lacZ", "lacZ", as.character(Genotype)))
RawData <- RawData %>% 
  mutate(Genotype = ifelse(as.character(Genotype) == "wtNDECDmSca", "Wild Type", as.character(Genotype)))
RawData <- RawData %>% 
  mutate(Genotype = ifelse(as.character(Genotype) == "NDECDmSca/\\TAD", "ΔTAD", as.character(Genotype)))
RawData <- RawData %>% 
  mutate(Genotype = ifelse(as.character(Genotype) == "NDECDmSca/\\TADPEST", "ΔTADΔPEST", as.character(Genotype)))
RawData <- RawData %>% 
  mutate(Genotype = ifelse(as.character(Genotype) == "NDECDmSca/\\PEST", "ΔPEST", as.character(Genotype)))
RawData <- RawData %>% 
  mutate(Genotype = ifelse(as.character(Genotype) == "NDECDmSca/\\OPA", "ΔOPA", as.character(Genotype)))
...
## Select Genotypes I want to plot
RawData <- filter(RawData,Genotype %in% c("lacZ","Wild Type","ΔTAD","ΔTADΔPEST","ΔPEST"))
...
ggplot(RawData, aes(x=Genotype, y=mBeta, fill = Genotype)) + theme_jmt() + theme(axis.title.x=element_blank()) +
  stat_summary(fun.y = mean, geom = "bar",show.legend = FALSE) + 
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.4, colour = "grey50") +
  geom_signif(comparisons = list(c("lacZ","Wild Type")), y_position = 0.062,
              map_signif_level=TRUE, test = "t.test", color = "black") +
  scale_fill_brewer(palette = "Set2") +
  labs(y = my_mB_title)
ggsave("mBeta expression of truncation constructs.jpg", device = "jpeg", dpi = "retina",
       width = 15, height = 12, units = "cm")

ggplot(RawData, aes(x=Genotype, y=m3, fill = Genotype)) + theme_jmt() + theme(axis.title.x=element_blank()) +
  stat_summary(fun.y = mean, geom = "bar",show.legend = FALSE) + 
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.4, colour = "grey50") +
  geom_signif(comparisons = list(c("lacZ","Wild Type")), y_position = 0.03,
              map_signif_level=TRUE, test = "t.test", color = "black") +
  geom_signif(comparisons = list(c("Wild Type","ΔTADΔPEST")), y_position = 0.034,
              map_signif_level=TRUE, test = "t.test", color = "black") +
  scale_fill_brewer(palette = "Set2") +
  labs(y = my_m3_title)
ggsave("m3 expression of truncation constructs.jpg", device = "jpeg", dpi = "retina",
       width = 15, height = 12, units = "cm")
...
ggplot(RawData, aes(x=Genotype, y=mBeta, fill = Genotype)) + theme_jmt() + theme(axis.title.x=element_blank()) +
  ...
  geom_signif(comparisons = list(c("lacZ","Wild Type")), y_position = 0.062,
              map_signif_level=TRUE, test = "t.test", color = "black") +
  geom_signif(comparisons = list(c("Wild Type","ΔPEST")), y_position = 0.068,
              map_signif_level=TRUE, test = "t.test", color = "black") +
  geom_signif(comparisons = list(c("Wild Type","ΔTAD")), y_position = 0.074,
              map_signif_level=TRUE, test = "t.test", color = "black") +
  geom_signif(comparisons = list(c("Wild Type","ΔTADΔPEST")), y_position = 0.080,
              map_signif_level=TRUE, test = "t.test", color = "black") +
  scale_fill_brewer(palette = "Set2") +
  labs(y = my_mB_title)
ggsave("mBeta expression of truncation constructs with all signif.jpg", device = "jpeg", dpi = "retina",
       width = 15, height = 12, units = "cm")
...
ggplot(RawData, aes(x=Genotype, y=m3, fill = Genotype)) + theme_jmt() + theme(axis.title.x=element_blank()) +
  ...
  geom_signif(comparisons = list(c("lacZ","Wild Type")), y_position = 0.03,
              map_signif_level=TRUE, test = "t.test", color = "black") +
  geom_signif(comparisons = list(c("Wild Type","ΔPEST")), y_position = 0.034,
              map_signif_level=TRUE, test = "t.test", color = "black") +
  geom_signif(comparisons = list(c("Wild Type","ΔTAD")), y_position = 0.038,
              map_signif_level=TRUE, test = "t.test", color = "black") +
  geom_signif(comparisons = list(c("Wild Type","ΔTADΔPEST")), y_position = 0.042,
              map_signif_level=TRUE, test = "t.test", color = "black") +
  scale_fill_brewer(palette = "Set2") +
  labs(y = my_m3_title)
ggsave("m3 expression of truncation constructs with all signif.jpg", device = "jpeg", dpi = "retina",
       width = 15, height = 12, units = "cm")