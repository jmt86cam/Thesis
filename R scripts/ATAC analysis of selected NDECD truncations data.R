...
setwd("/Users/jonathantownson/Documents/PhD/ATAC data/NDECD truncations")
# Instruct the script of any Genotype ordering required e.g. control then experimental condition
XAxisOrder <- c("lacZ", "Wild Type")

#### Read in the data ####
RawData <- read.csv("/Users/jonathantownson/Documents/PhD/ATAC data/NDECD truncations/ATAC qPCR selected data for analysis.csv")
...
ggplot(m3enhancer, aes(x=Genotype, y=Closed.control.normalised, fill = Genotype)) + theme_jmt() + 
  ...
  geom_signif(comparisons = list(c("lacZ","Wild Type")), y_position = 115,
              map_signif_level=TRUE, test = "t.test", color = "black") +
  geom_signif(comparisons = list(c("Wild Type","ΔTADΔPEST")), y_position = 125,
              map_signif_level=TRUE, test = "t.test", color = "black") +
...
ggplot(mBetaenhancer, aes(x=Genotype, y=Closed.control.normalised, fill = Genotype)) + theme_jmt() + 
  ...
  stat_summary(fun.y = mean, geom = "bar",show.legend = FALSE) + 
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.4, colour = "grey50") +
  geom_signif(comparisons = list(c("lacZ","Wild Type")), y_position = 75,
              map_signif_level=TRUE, test = "t.test", color = "black") +
  geom_signif(comparisons = list(c("Wild Type","ΔTADΔPEST")), y_position = 80,
              map_signif_level=TRUE, test = "t.test", color = "black") +
  scale_fill_brewer(palette = "Set2") +
  labs(y = my_mB_title)
