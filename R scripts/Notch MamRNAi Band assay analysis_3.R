...
mainDir <- "/Users/jonathantownson/Documents/PhD/Images/Analysed image data/Band assay/Truncations/MamRNAi/NDECD"
setwd(mainDir)
# Load the background values
DF <- read.csv("/Users/jonathantownson/Documents/PhD/Images/Analysed image data/Band assay/Truncations/MamRNAi/NDECD/Background NDECD with MamRNAi levels.csv", head = TRUE, sep = ",")
BackgroundValues <- data.frame(Nucleus=DF[,1], NDECDBG=rowMeans(DF[,-1]))
rm(DF)
# Create the path where the data is stored
DataPath <- "/Users/jonathantownson/Documents/PhD/Images/Analysed image data/Band assay/Truncations/MamRNAi/NDECD/Band assay files"
#DataPath <- "/Users/jonathantownson/Documents/PhD/Images/Analysed image data/Band assay/Truncations/MamRNAi/NDECD/Individual Genotype comparisons csv files/NDECDPEST"
...
for (p in 1:length(MeanGenotypesByDateRawData)) {
  ...
  band_plot(df, Labs) + theme(legend.position = "bottom", legend.title = element_blank()) +
    scale_y_continuous(breaks = seq(700,1200, by = 100), limits = c(700,1200))
...
for (p in 1:length(RelIntNDECDMinSixSubSepDates)) {
  ...
  RelInt_crossbar(df, Labs) + theme(legend.position = "none") +
    geom_signif(comparisons = list(c(names(RelIntNDECDMinSixSubSepDates)[p],"Wild Type")), 
                map_signif_level=TRUE, test = "t.test", color = "black", y_position = 450) +
...
df <- filter(RelIntNDECD,Genotype %in% c("wtNDECD","TADPEST"))
...
df <- filter(RelIntNDECDBg,Genotype %in% c("wtNDECD","TADPEST"))
RelInt_crossbar(df,Labs) +
  geom_signif(comparisons = list(c("wtNDECD","TADPEST")), 
              map_signif_level=TRUE, test = "t.test", color = "black")
#### end ####
