...
mainDir <- "/Users/jonathantownson/Documents/PhD/FRAP data/Dimerisation mutant comparison"
setwd(mainDir)
# Create the path where the data is stored
DataPath <- "/Users/jonathantownson/Documents/PhD/FRAP data/Dimerisation mutant comparison/CSV FILES"
...
WTmodelOne <- function(dft){
  ModelData <- 0.8364*(1-exp(-0.08534*dft))
  return(ModelData)
}
R2007AmodelOne <- function(dft){
  ModelData <- 0.8135*(1-exp(-0.0976*dft))
  return(ModelData)
}
...
WTmodelTwo <- function(dft){
  ModelData <- (0.8843*37.11*.01)*(1-exp(-0.3858*dft)) + (0.8843*(100-37.11)*.01)*(1-exp(-0.04107*dft))
  return(ModelData)
}
R2007AmodelTwo <- function(dft){
  ModelData <- (0.8730*46.73*.01)*(1-exp(-0.3175*dft)) + (0.8730*(100-46.73)*.01)*(1-exp(-0.03643*dft))
  return(ModelData)
}
#### end ####
...
for (f in 1:length(FolderNames)){
  ...
  for (i in 1:length(Files)){
    ...
    df$Nucleus <- paste(substr(FolderNames[f], nchar(FolderNames[f])-5, nchar(FolderNames[f])), Files[i], sep = "_")
    df$Genotype <- str_sub(basename(Files[[i]]), end=-10)
    SubFiles[[i]] <- df
    names(SubFiles)[i] <- paste(substr(FolderNames[f], nchar(FolderNames[f])-5, nchar(FolderNames[f])), Files[i], sep = "_")
    rm(df,BleachedDNV)
  } 
  RawData[[f]] <- SubFiles
...
ModelDataOne$R2007A <- R2007AmodelOne(ModelDataOne$`Time (s)`)
...
ModelDataTwo$R2007A <- R2007AmodelTwo(ModelDataTwo$`Time (s)`)
...
df <- subset(MeanDataMinusPreBleach, Genotype == "WT" | Genotype == "R2007A")
recovery_plot_OnePhase(df,Labs) + geom_line(aes(y=ModelOneValues), size = 0.4, colour = "black")
ggsave("Wild type vs  R2007A.jpg", device = "jpeg", dpi = "retina",
