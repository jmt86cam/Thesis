...
WTvsMutByDate <- function(SeparatedWTdata, AllData){
  ...
  for (l in 1:length(SeparatedWTdata)) {
    ...
    df <- rbind(WT,Mutant)
    WTnumber <- ncol(SeparatedWTdata[[l]])-1
    Mutnumber <- ncol(AllData[[Genotype]])-1
    df <- df %>% mutate(Genotype = fct_relevel(Genotype, c(paste("Wild Type (N = ", WTnumber, ")", sep = ""), paste(names(SeparatedWTdata)[l], " (N = ", Mutnumber, ")", sep = ""))))
    OutputList[[l]] <- df
    names(OutputList)[l] <- names(SeparatedWTdata[l])
    rm(df, WT, Mutant,WTnumber,Mutnumber)
  }
  return(OutputList)
}
...
RelIntSepDates <- function(RelIntDataFrame, GenotypeToSep) {
  ...
  for (t in 1:length(unique(Mutants$Genotype))) {
    ...
    df <- df %>% mutate(Genotype = fct_relevel(Genotype, c("NΔECD",unique(Mutants$Genotype)[t])))
    OutputList[[t]] <- df
...
mainDir <- "/Users/jonathantownson/Documents/PhD/Images/Analysed image data/Band assay/NDECDmSc vs R2007AmSca vs NICDmSca/Su(H)"
setwd(mainDir)
# Load the background values
DF <- read.csv("/Users/jonathantownson/Documents/PhD/Images/Analysed image data/Band assay/NDECDmSc vs R2007AmSca vs NICDmSca/Su(H)/Background SuH levels.csv", head = TRUE, sep = ",")
BackgroundValues <- data.frame(Nucleus=DF[,1], SuHBG=rowMeans(DF[,-1]))
rm(DF)
# Create the path where the data is stored
DataPath <- "/Users/jonathantownson/Documents/PhD/Images/Analysed image data/Band assay/NDECDmSc vs R2007AmSca vs NICDmSca/Su(H)/Band assay csv files"
...
WTSuHRDSepDates <- SepDates(SuHRawData,"NΔECD")
WTSuHminsixSubSepDates <- SepDates(SuHminsixSub,"NΔECD")
...
RelIntSuHSepDates <- (RelIntSepDates(RelIntSuH, "NΔECD"))
...
RelIntSuHMinSixSubSepDates <- RelIntSepDates(RelIntSuHMinSixSub, "NΔECD")
...
RelIntSuHBgSepDates <- RelIntSepDates(RelIntSuHBg, "NΔECD")
...
RelIntSuHBg0SepDates <- RelIntSepDates(RelIntSuHBg0, "NΔECD")
...
RelIntChangeSuHBgSepDates <- RelIntSepDates(RelIntChangeSuHBg, "NΔECD")
...
RelIntChangeSuHSepDates <- (RelIntSepDates(RelIntChangeSuH, "NΔECD"))
#### end ####
...
for (p in 1:length(RelIntSuHSepDates)) {
  ...
  RelInt_crossbar(df, Labs) + theme(legend.position = "none") +
    geom_signif(comparisons = list(c(names(RelIntSuHSepDates)[p],"NΔECD")), 
...
for (p in 1:length(RelIntSuHBgSepDates)) {
  ...
  RelInt_crossbar(df, Labs) + theme(legend.position = "none") +
    geom_signif(comparisons = list(c(names(RelIntSuHBgSepDates)[p],"NΔECD")), 
...
for (p in 1:length(RelIntSuHBg0SepDates)) {
  ...
  RelInt_crossbar(df, Labs) + theme(legend.position = "none") +
    geom_signif(comparisons = list(c(names(RelIntSuHBg0SepDates)[p],"NΔECD")), 
                map_signif_level=TRUE, test = "t.test", color = "black")
...
for (p in 1:length(RelIntChangeSuHBgSepDates)) {
  ...
  RelInt_crossbar(df, Labs) + theme(legend.position = "none") +
    geom_signif(comparisons = list(c(names(RelIntChangeSuHBgSepDates)[p],"NΔECD")), 
                map_signif_level=TRUE, test = "t.test", color = "black", y_position = 4.7) +
    scale_y_continuous(breaks = seq(0, 5, by = 1), limits = c(0,5))
...
for (p in 1:length(RelIntChangeSuHSepDates)) {
  ...
  RelInt_crossbar(df, Labs) + theme(legend.position = "none") +
    geom_signif(comparisons = list(c(names(RelIntChangeSuHSepDates)[p],"NΔECD")), 
                map_signif_level=TRUE, test = "t.test", color = "black") 
...
for (p in 1:length(RelIntSuHMinSixSubSepDates)) {
  ...
  RelInt_crossbar(df, Labs) + theme(legend.position = "none") +
    geom_signif(comparisons = list(c(names(RelIntSuHMinSixSubSepDates)[p],"NΔECD")), 
