...
mainDir <- "/Users/jonathantownson/Documents/PhD/Images/Analysed image data/Band assay/Truncations/Su(H)"
setwd(mainDir)
...
WTSuHRDSepDates <- SepDates(SuHRawData,"Wild Type")
WTSuHminsixSubSepDates <- SepDates(SuHminsixSub,"Wild Type")

MeanGenotypesByDateRawData <- WTvsMutByDate(WTSuHRDSepDates, SuHRawData)
MeanGenotypesByDateMinSix <- WTvsMutByDate(WTSuHminsixSubSepDates, SuHminsixSub)
...
RelIntSuHSepDates <- (RelIntSepDates(RelIntSuH, "Wild Type"))
...
RelIntSuHEndsSepDates <- (RelIntSepDates(RelIntSuHEnds, "Wild Type"))
...
RelIntSuHMinSixSubSepDates <- RelIntSepDates(RelIntSuHMinSixSub, "Wild Type")
...
for (s in 1:length(SuHRawData)) {
  ...
  for (t in 1:length(ColNames)) {
    ...
    rm(Band, Background, BandSubBg)
    RelIntSuHBg0 <- rbind(RelIntSuHBg0,RelInt)
  }
  rm(RelInt, BandValues, df, ColNames)
}
RelIntSuHBg0SepDates <- RelIntSepDates(RelIntSuHBg0, "Wild Type")
...
RelIntSuHBgSubSepDates <- RelIntSepDates(RelIntSuHBgSub, "Wild Type")
...
RelIntChangeSuHSepDates <- (RelIntSepDates(RelIntChangeSuH, "Wild Type"))
#### end ####
...
NormDis_plots(RelIntSuHSepDates, "RelativeBandIntensity", "Genotype","Normal distribution of RelInt plots")
NormDis_plots(RelIntSuHBgSepDates, "RelativeBandIntensity", "Genotype","Normal distribution of RelIntBg plots")
NormDis_plots(RelIntSuHBg0SepDates, "RelativeBandIntensity", "Genotype","Normal distribution of RelIntBg0 plots")
NormDis_plots(RelIntSuHBgSubSepDates, "RelativeBandIntensity", "Genotype","Normal distribution of RelIntBgSub plots")
NormDis_plots(RelIntSuHMinSixSubSepDates, "RelativeBandIntensity", "Genotype","Normal distribution of RelIntMinSixSub plots")
NormDis_plots(RelIntChangeSuHSepDates, "RelativeBandIntensity", "Genotype","Normal distribution of RelIntChange plots")
NormDis_plots(RelIntChangeSuHBgSepDates, "RelativeBandIntensity", "Genotype","Normal distribution of RelIntChangeBg plots")
...
QQ_plots(RelIntSuHSepDates, "RelativeBandIntensity", "Genotype","QQ Plots of RelInt")
QQ_plots(RelIntSuHBgSepDates, "RelativeBandIntensity", "Genotype","QQ Plots of RelIntBg")
QQ_plots(RelIntSuHBg0SepDates, "RelativeBandIntensity", "Genotype","QQ Plots of RelIntBg0")
QQ_plots(RelIntSuHBgSubSepDates, "RelativeBandIntensity", "Genotype","QQ Plots of RelIntBgSub")
QQ_plots(RelIntSuHMinSixSubSepDates, "RelativeBandIntensity", "Genotype","QQ Plots of RelIntBgSub")
QQ_plots(RelIntChangeSuHSepDates, "RelativeBandIntensity", "Genotype","QQ Plots of RelIntChange")
QQ_plots(RelIntChangeSuHBgSepDates, "RelativeBandIntensity", "Genotype","QQ Plots of RelIntChangeBg")
...
SumRelIntSuH <- DataStatSumTestListFun(RelIntSuHSepDates,"Summary of Su(H) RelInt")
SumRelIntSuHBg <- DataStatSumTestListFun(RelIntSuHBgSepDates,"Summary of Su(H) RelInt Bg")
SumRelIntSuHBg0 <- DataStatSumTestListFun(RelIntSuHBg0SepDates,"Summary of Su(H) RelInt Bg0")
SumRelIntSuHBgSub <- DataStatSumTestListFun(RelIntSuHBgSubSepDates,"Summary of Su(H) RelInt BgSub")
SumRelIntSuHMinSixSub <- DataStatSumTestListFun(RelIntSuHMinSixSubSepDates,"Summary of Su(H) RelInt MinSixSub")
SumRelIntChangeSuH <- DataStatSumTestListFun(RelIntChangeSuHSepDates,"Summary of Su(H) RelIntChange")
SumRelIntChangeSuHBg <- DataStatSumTestListFun(RelIntChangeSuHBgSepDates,"Summary of Su(H) RelInt BgChange")
...
SigStatsRelIntSuH <- Stats_tests(RelIntSuHSepDates,"SigStats of Su(H) RelInt")
SigStatsRelIntSuHBg <- Stats_tests(RelIntSuHBgSepDates,"SigStats of Su(H) RelInt Bg")
SigStatsRelIntSuHBg0 <- Stats_tests(RelIntSuHBg0SepDates,"SigStats of Su(H) RelInt Bg0")
SigStatsRelIntSuHBgSub <- Stats_tests(RelIntSuHBgSubSepDates,"SigStats of Su(H) RelInt BgSub")
SigStatsRelIntSuHMinSixSub <- Stats_tests(RelIntSuHMinSixSubSepDates,"SigStats of Su(H) RelInt MinSixSub")
SigStatsRelIntChangeSuH <- Stats_tests(RelIntChangeSuHSepDates,"SigStats of Su(H) RelIntChange")
SigStatsRelIntChangeSuHBg <- Stats_tests(RelIntChangeSuHBgSepDates,"SigStats of Su(H) RelIntChange Bg")
...
for (p in 1:length(RelIntSuHSepDates)) {
  Labs <- c(title = " ", x = " ", y = "Fold change of Su(H)::EGFP intensity")
  df <- na.omit(RelIntSuHSepDates[[p]])
  RelInt_crossbar(df, Labs) + theme(legend.position = "none") +
    geom_signif(comparisons = list(c(names(RelIntSuHSepDates)[p],"Wild Type")), 
                map_signif_level=TRUE, test = "t.test", color = "black") 
  ggsave(paste("RelInt plot of ", names(RelIntSuHSepDates)[p], ".jpg"), device = "jpeg", dpi = "retina",
         width = 12, height = 12, units = "cm")
}

for (p in 1:length(RelIntChangeSuHSepDates)) {
  Labs <- c(title = " ", x = " ", y = "Relative change of Su(H)::EGFP intensity")
  df <- na.omit(RelIntChangeSuHSepDates[[p]])
  RelInt_crossbar(df, Labs) + theme(legend.position = "none") +
    geom_signif(comparisons = list(c(names(RelIntChangeSuHSepDates)[p],"Wild Type")), 
                map_signif_level=TRUE, test = "t.test", color = "black") 
  ggsave(paste("RelIntChange plot of ", names(RelIntChangeSuHSepDates)[p], ".jpg"), device = "jpeg", dpi = "retina",
         width = 12, height = 12, units = "cm")
}

for (p in 1:length(RelIntSuHBgSepDates)) {
  Labs <- c(title = " ", x = " ", y = "Fold change of Su(H)::EGFP intensity")
  df <- na.omit(RelIntSuHBgSepDates[[p]])
  RelInt_crossbar(df, Labs) + theme(legend.position = "none") +
    geom_signif(comparisons = list(c(names(RelIntSuHBgSepDates)[p],"Wild Type")), 
                map_signif_level=TRUE, test = "t.test", color = "black", y_position = 5.7) +
    scale_y_continuous(breaks = seq(0, 6, by = 1), limits = c(0,6))
  ggsave(paste("Fold change plot of ", names(RelIntSuHBgSepDates)[p], ".jpg"), device = "jpeg", dpi = "retina",
         width = 12, height = 12, units = "cm")
}

for (p in 1:length(RelIntChangeSuHBgSepDates)) {
  Labs <- c(title = " ", x = " ", y = "Relative change of Su(H)::EGFP intensity")
  df <- na.omit(RelIntChangeSuHBgSepDates[[p]])
  RelInt_crossbar(df, Labs) + theme(legend.position = "none") +
    geom_signif(comparisons = list(c(names(RelIntChangeSuHBgSepDates)[p],"Wild Type")), 
                map_signif_level=TRUE, test = "t.test", color = "black", y_position = 4.7) +
    scale_y_continuous(breaks = seq(0, 5, by = 1), limits = c(0,5))
  ggsave(paste("RelIntChangeBg plot of ", names(RelIntChangeSuHBgSepDates)[p], ".jpg"), device = "jpeg", dpi = "retina",
         width = 12, height = 12, units = "cm")
}

for (p in 1:length(RelIntSuHBg0SepDates)) {
  Labs <- c(title = " ", x = " ", y = "Fold change of Su(H)::EGFP intensity")
  df <- na.omit(RelIntSuHBg0SepDates[[p]])
  RelInt_crossbar(df, Labs) + theme(legend.position = "none") +
    geom_signif(comparisons = list(c(names(RelIntSuHBg0SepDates)[p],"Wild Type")), 
                map_signif_level=TRUE, test = "t.test", color = "black")
  ggsave(paste("0 plot of ", names(RelIntSuHBg0SepDates)[p], ".jpg"), device = "jpeg", dpi = "retina",
         width = 12, height = 12, units = "cm")
}

for (p in 1:length(RelIntSuHBgSubSepDates)) {
  Labs <- c(title = " ", x = " ", y = "Su(H)::EGFP Fluorescence (AU)")
  df <- na.omit(RelIntSuHBgSubSepDates[[p]])
  RelInt_crossbar(df, Labs) + theme(legend.position = "none") +
    geom_signif(comparisons = list(c(names(RelIntSuHBgSubSepDates)[p],"Wild Type")), 
                map_signif_level=TRUE, test = "t.test", color = "black")
  ggsave(paste("Band minus Bg plot of ", names(RelIntSuHBgSubSepDates)[p], ".jpg"), device = "jpeg", dpi = "retina",
         width = 12, height = 12, units = "cm")
}
for (p in 1:length(RelIntSuHMinSixSubSepDates)) {
  Labs <- c(title = " ", x = " ", y = "Su(H)::EGFP Fluorescence (AU)")
  df <- na.omit(RelIntSuHMinSixSubSepDates[[p]])
  RelInt_crossbar(df, Labs) + theme(legend.position = "none") +
    geom_signif(comparisons = list(c(names(RelIntSuHMinSixSubSepDates)[p],"Wild Type")), 
                map_signif_level=TRUE, test = "t.test", color = "black", y_position = 2700) +
    scale_y_continuous(breaks = seq(0, 3000, by = 500), limits = c(0,3000))
  ggsave(paste("Band minus MinSix plot of ", names(RelIntSuHMinSixSubSepDates)[p],".jpg"), device = "jpeg", dpi = "retina",
         width = 12, height = 12, units = "cm")
}
## Plot individual bands and save plots
for (p in 1:length(names(SuHRawData))) {
  Labs <- c(title = paste("All ",names(SuHRawData)[p]," Raw Data"), x = "Distance (um)", y = "SuH::GFP Fluorescence (AU)")
  df <- SuHRawData[[p]]
  ...
  ggplot(df, aes(Distance,value)) + theme_jmt() +
    ...
    scale_color_brewer(palette = "Set2")
  ggsave(paste("All ",names(SuHRawData)[p]," Raw Data", ".pdf"), device = "pdf", width = 45, height = 5*(ceiling(PlotNumber/5)), units = "cm", dpi = "retina")
  rm(df,PlotNumber)
}
...
Labs <- c(title = "Raw Data", x = "Distance (um)", y = "SuH::GFP Fluorescence (AU)")
df <- filter(MeanSuHRawData,Name %in% c("wtNDECD","NDECDTADPEST","NDECDPEST","NDECDTAD","NDECDOPA"))
band_plot(df, Labs)

## Plots of the MinSix data
Labs <- c(title = "Subtract minimum six values", x = "Distance (um)", y = "SuH::GFP Fluorescence (AU)")
df <- filter(MeanSuHMinSixSub,Name %in% c("wtNDECD","NDECDTADPEST","NDECDPEST","NDECDTAD","NDECDOPA"))
band_plot(df, Labs)

## Plots of the BgNorm data
Labs <- c(title = "Normalised to background value", x = "Distance (um)", y = "Normalised SuH Fluorescence")
df <- filter(MeanSuHBgNorm,Name %in% c("wtNDECD","NDECDTADPEST","NDECDPEST","NDECDTAD","NDECDOPA"))
band_plot(df, Labs)

## Compare the WT control conditions
Labs <- c(title = "Wild Type Raw Data", x = "Distance (um)", y = "Su(H)::GFP Fluorescence (AU)")
df <- MeanDF(WTSuHRDSepDates)
band_plot(df, Labs) 

## RelInt plots
Labs <- c(title = "Relative intensity of band", x = "Genotype", y = "Relative SuH intensity")
df <- filter(RelIntSuH,Name %in% c("wtNDECD","NDECDTADPEST","NDECDPEST","NDECDTAD","NDECDOPA"))
RelInt_crossbar(df,Labs)

Labs <- c(title = "Fold change of band over background", x = "Genotype", y = "SuH fold change")
df <- filter(RelIntSuHBg,Name %in% c("wtNDECD","NDECDTADPEST","NDECDPEST","NDECDTAD","NDECDOPA"))
RelInt_crossbar(df,Labs)
